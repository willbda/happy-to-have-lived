...
#### **MilestoneRepository** (Simple)
```
Query Pattern: Simple JOIN (no aggregation)
Relations: 1 (Expectation join)
Methods:
  - fetchAll()                    # All milestones with expectations
  - fetchForExport()              # Milestones filtered by date
  - exists(id:)                   # Check existence
```

**SQL Strategy**: Single JOIN query with expectations:
```sql
SELECT m.*, e.*
FROM milestones m
...
**Performance**: O(1) database round trip, simple 1:1 join

#### **ObligationRepository** (Simple)
```
Query Pattern: Simple JOIN (no aggregation)
Relations: 1 (Expectation join)
Methods:
  - fetchAll()                    # All obligations with expectations
  - fetchForExport()              # Obligations filtered by date
  - exists(id:)                   # Check existence
```
...
| **Query Builder JOIN** | TimePeriod, Milestone, Obligation | Simple 1:1 relationships |
...
milestones + expectations → MilestoneWithDetails → MilestoneRowView
  (Simple JOIN assembled)

obligations + expectations → ObligationWithDetails → ObligationRowView
  (Simple JOIN assembled)
```
...
| **MilestonesListViewModel** | MilestoneRepository | All milestones with checkpoints |
| **ObligationsListViewModel** | ObligationRepository | All obligations with deadlines |
...
#### **MilestoneRowView**
```swift
Receives: MilestoneWithDetails (milestone + expectation)
Displays:
...
#### **ObligationRowView**
```swift
Receives: ObligationWithDetails (obligation + expectation)
Displays:
  - Title (from expectation)
  - Deadline
...
#### Milestones Display Order
- `fetchAll()`: Ordered by `targetDate ASC` (nearest checkpoints first)

#### Obligations Display Order
- `fetchAll()`: Ordered by `deadline ASC` (most urgent deadlines first)
...
2. **Repository pattern** for all entities (Goals, Milestones, Obligations, Actions, Terms, Values)
3. **Complete Expectation type infrastructure**:
   - Goals: Full progress tracking capability (date ranges, measures, values, term assignments)
   - Milestones: Point-in-time checkpoints with target dates
   - Obligations: External commitments with deadlines and requesters
4. **Status tracking** on GoalTerms (planned, active, completed, etc.)
5. **Active goal detection** (targetDate NULL or future)
6. **Measurement targets** (ExpectationMeasure)
7. **Measurement actuals** (MeasuredAction, ActionGoalContribution)
8. **List views** with proper ViewModel pattern (Goals, Milestones, Obligations, Actions, Terms, Values)
9. **Form views** for creating/editing all expectation types
10. **Coordinators** for atomic writes (Goals, Milestones, Obligations, Actions, Terms, Values, Measures)
11. **Validation layer** with two-phase validation for all types
12. **Error mapping** and user-friendly messages
...
Obligation Display:
  ...
  ⏳ Status badges (pending, approaching, overdue)
...
### Models
- `/Sources/Models/Abstractions/` - Base entities (Action, Expectation, Measure, PersonalValue, TimePeriod)
- `/Sources/Models/Basics/` - Working entities (Goal, Milestone, Obligation, Term)
- `/Sources/Models/Composits/` - Junction tables (ExpectationMeasure, MeasuredAction, ActionGoalContribution, etc.)
- `/Sources/Models/DataTypes/` - Canonical types (GoalData, ActionData, TimePeriodData, MilestoneWithDetails, ObligationWithDetails)

### Services
- `/Sources/Services/Repositories/` - Read operations (GoalRepository, MilestoneRepository, ObligationRepository, etc.)
- `/Sources/Services/Coordinators/` - Write operations (GoalCoordinator, MilestoneCoordinator, ObligationCoordinator, etc.)
- `/Sources/Services/Coordinators/FormData/` - Form data structures (GoalFormData, MilestoneFormData, ObligationFormData, ExpectationFormDataBase)
- `/Sources/Services/Validation/` - Business rule validation (ValidationRules with GoalValidation, MilestoneValidation, ObligationValidation)

### ViewModels
- `/Sources/App/ViewModels/ListViewModels/` - List ViewModels (GoalsListViewModel, MilestonesListViewModel, ObligationsListViewModel, etc.)
- `/Sources/App/ViewModels/FormViewModels/` - Form ViewModels (GoalFormViewModel, MilestoneFormViewModel, ObligationFormViewModel, etc.)

### Views
- `/Sources/App/Views/ListViews/` - List views (GoalsListView, MilestonesListView, ObligationsListView, etc.)
- `/Sources/App/Views/FormViews/` - Form views (GoalFormView, MilestoneFormView, ObligationFormView, etc.)
- `/Sources/App/Views/RowViews/` - Row views (GoalRowView, MilestoneRowView, ObligationRowView, ActionRowView, TermRowView)
